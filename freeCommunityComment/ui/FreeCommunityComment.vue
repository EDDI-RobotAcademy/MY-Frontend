    ...
    <div class="comments-list">
      <div v-for="comment in comments" :key="comment.commentId" class="comment-item">
        <div class="comment-main">
          ...
          <ReplyToggle :repliesCount="comment.replies.length" @toggle="toggleReplies(comment.commentId)" />
          <div v-if="showReplies[comment.commentId]">
            <ReplyForm @submit="submitReply(comment.commentId)" />
...
const showReplies = ref<{ [key: number]: boolean }>({})
...
const submitReply = async (commentId: number, replyText: string) => {
  const userToken = localStorage.getItem("userToken")
  if (!userToken) {
    alert('로그인이 필요한 서비스입니다.')
    return
  ...
  try {
    ...
    await freeCommunityCommentStore.addFreeCommunityComment(replyData) // Store 메서드 재사용
    await loadComments() // 댓글 목록 다시 로드
  } catch (error) {
