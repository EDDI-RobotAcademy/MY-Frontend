<template>
    <div class="container">
        ...
        <h1 class="title">{{ followersCount }}명의 팔로워</h1>

        <div class="follower-container">
            <div v-for="(follower, index) in followersList" :key="follower.id" class="follower-item">
...
const getFollowerNicknames = async (followersString) => {
    ...
    const followerIds = followersString.split(',').map(id => id.trim());

    try {
        const userDataList = await accountStore.requestGetUserProfileByAccountIdToDjango(followerIds);

        followersList.value = userDataList.map(userData => ({
        ...
        // 에러 시 기본값으로 설정
        followersList.value = followerIds.map(id => ({
            id: id,
