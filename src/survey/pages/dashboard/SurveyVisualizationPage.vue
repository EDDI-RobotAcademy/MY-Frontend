<template>
    <div class="dashboard-container">
      <div class="title">
        <h2>설문조사 응답 분석</h2>
        <p>설문조사 ID를 입력하여 각 질문의 선택지 비율을 확인할 수 있습니다.</p>
      </div>
      ...
      <div class="filter-section">
        ...
        <button @click="loadSurveyAnswers" class="request-button">답변 조회</button>
      </div>
  
      <!-- 설문조사 질문별 답변 그래프 및 서술형 나열 -->
  ...
  export default {
    ...
    methods: {
      ...mapActions('surveyModule', ['requestListSurveyAnswerToDjango']),
  
      async loadSurveyAnswers() {
        if (!this.surveyId) {
          alert('설문조사 ID를 입력하세요.');
          return;
        }
        ...
        try {
          const response = await this.requestListSurveyAnswerToDjango(payload);
      ...
      getQuestionType(answer) {
        ...
        if (answer.survey_custom_selection_text || answer.survey_fixed_five_score_selection || answer.survey_fixed_boolean_selection) {
